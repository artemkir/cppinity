...
public:
	...
	// Size in relative viewport coords (0 to 1, e.g., 1.0 = 100% screen width/height), if -1, maintain aspect ratio
    // Position is anchor point (0 to 1, e.g., 0.5, 0.5 = centered)
    // anchor_type: "TopLeft", "Center" - affects how pos is interpreted
	// canvas_size: Virtual resolution (e.g., 1920x1080)
	...
	Vector2 GetAdjustedRelativeCanvasSize() const; // Adjusted size considering aspect ratio if -1
